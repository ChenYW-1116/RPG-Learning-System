...
請執行以下兩個前置任務以準備分析素材：
1. **規格轉意圖 (Spec to Intent)**：讀取 {samples/001-ielts-wsc_opt_phase1_output_spec.md}，使用下方 `{意圖描述}` 模板將其轉換為結構化的規格意圖文件，輸出為 `{intent1.md}`。
2. **代碼轉意圖 (Code to Intent)**：讀取 {samples/IELTS_Writing-Sprint_Coach.html}，結合 `.agent/程式碼逆向工程提示詞.md` 的指引進行逆向分析，並使用相同的 `{意圖描述}` 模板將代碼現狀轉換為實作意圖文件，輸出為 `{intent2.md}`。 
...
該提示詞應包含：
1. **背景**：告知 AI 這是一份需要修復的 {samples/IELTS_Writing-Sprint_Coach.html} 代碼。
